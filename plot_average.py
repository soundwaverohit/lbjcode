...
names = ["AIN0", "DAC0"]
intervalHandle = 1
loopAmount = 1 #for number of points collected per frequency
...
while count< num_plots+1:
    ...
    for i in range(start_frequency, stop_frequency, step_size):
        ...
        print("Frequency ", i, " is set and intensity data values collected ", loopAmount, " times and intensity isL ", get_avg(temp_intensity) )
...
plt.plot(final_vals["Frequency"], normalized_intensities)
plt.xlabel("Frequency")
plt.ylabel("Intensity")
plt.title("Intensity vs Frequency Graph")
plt.show()
